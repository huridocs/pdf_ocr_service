...
class TestPdfFile(TestCase):
    def test_save(self):
        ...
        shutil.rmtree(f'{DOCKER_VOLUME_PATH}/source_pdfs/{tenant}', ignore_errors=True)

        pdf_file = PdfFile(tenant)
        with open(f'{ROOT_FOLDER}/test_files/test.pdf', 'rb') as file:
            pdf_file.save(pdf_file_name, file.read())

        self.assertTrue(os.path.exists(f'{DOCKER_VOLUME_PATH}/source_pdfs/tenant_one/test.pdf'))
        shutil.rmtree(f'{DOCKER_VOLUME_PATH}/source_pdfs/{tenant}', ignore_errors=True)
    ...
    def test_save_different_file(self):
        ...
        self.assertTrue(os.path.exists(f'{DOCKER_VOLUME_PATH}/source_pdfs/{tenant}/{pdf_file_name}'))
